...
use crate::{
    aabb::Aabb, bounded::Bounded, bvh_builder::BvhBuilder, bvh_config::BvhConfig, hit::Hit, ray::Ray, traceable::Traceable,
};
...
impl<T: RealField + Copy + ToPrimitive> Bvh<T> {
        ...
        B: Bounded<T> + Traceable<T>,
    {
        ...
        if node.count > 0 {
            ...
            let mut closest_distance = T::max_value().unwrap();
    ...
    fn intersect_any_recursive<B>(&self, ray: &Ray<T>, shapes: &[B], node_index: usize, max_distance: T) -> bool
    where
        B: Bounded<T> + Traceable<T>,
    {
        if node_index >= self.nodes.len() {
        ...
        self.intersect_any_recursive(ray, shapes, left_child_index, max_distance)
